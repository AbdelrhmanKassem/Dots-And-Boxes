...
//a function that takes user input while he's playing to determine which action to take
void take_input(void)
{
    ...
    if(scanf("%d %d",&n,&m)!=2 || buf[4]>1){
            ...
            if((buf[0]=='U'|| buf[0]=='u')&& buf[1]=='\n') {undomove=false; undo();}
                                                                   ...
                                                                   else{printf("\t\tNo Undone Moves To Redo!!\n");
                                                                        take_input();}}
...
//if a user input in valid this function modifies the arrays to cope with the changes
...
//a function that is executed after checking if a box has been closed by a player
...
//a function that coordinates box checking to only check in the right places
void check_box (void)
{
    t=2;
    box=0;
...
//checks for the box right under the last line played
...
//checks for the box right above the last line played
...
//checks for the box to the right of the last line played
...
//checks for the box to the left of the last line played
...
//function that generates the input for the Ai mode
void ai_input (void)
{
...
//function that save every move done by the user to be able to undo them
void save_move (void)
{
...
//the function that undoes the last move
void undo (void)
{
    if(total_moves>0){
    undone=true;
...
//calculating each user's score after each turn
...
//function that undoes one extra move when playing against the ai so that the turn can return to the main player
void ai_undo (void)
{
if(total_moves>0){
    undone=true;
...
//function that redoes the last undone move
void redo (void)
{
    lines[n][m]=1;
    if(turn==1){
