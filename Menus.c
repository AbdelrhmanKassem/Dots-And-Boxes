...
extern struct user_rank a[10],z; //array of structs to store the top 10 users

//function to display the main menu then takes an input from the user to send him to the next menu
void main_menu (void)
{
    int s;
    char buf [256]={0};
...
//menu to select the difficulty which sets the variable grid_s
...
// menu to select whether you will play against the PC or in a 2 player mode
...
//prints invalid input in red for aesthetics
...
// functions that draws the game grid using 2 arrays to know what exactly to print in each place on the screen
void draw_grid (void)
{
    calc_score();
    ...
    if(rem_lines>=1)
    {printf("\n\n\033[1;36m\t\t\tEnter 'U' For Undo, 'R' For Redo, 'S' For Save, And 'E' To Go To Main Menu\n\n");}
    if (vsai==0){
        if (turn==1){
            printf("\033[1;32m\t\t\t\t\tTime: %d : %d     Remaining Lines:%d \033[1;31m Turn: %s    \n\n",T_now_mins,T_now_secs%60,rem_lines,p1.name);
            }
        else{
            printf("\033[1;32m\t\t\t\t\tTime: %d : %d     Remaining Lines:%d \033[1;34m Turn: %s    \n\n",T_now_mins,T_now_secs%60,rem_lines,p2.name);
            }
...
//prints a row of horizontal numbers while keeping the spaces between each number equal
    printf("\t\t\t\t\t0   ");
 ...
 //loops around each row and column to print lines with color or just leave them empty
    int j;
...
//for when no more lines can be placed
    if(grid_s==5){
...
////draws an empty vertical line

void ev (void)
{
    printf(" ");
}
////draws an empty horizontal line
...
//colors the box with p1 color and name
void boxp1 (void)
{
...
//colors the box with p2 color and name
...
//sets the arrays and some variable to 0 for a new game or so
...
//sets the struct variable to zero for a new game
...
//function that displays after the game has bee done and takes the user name and compares it with the top 10 list to see if he qualifies
void game_over (void)
{
    load_top10();
    calc_score();
...
//sorting and displaying the top 10 users
void top_10 (void)
{
    char buf [256]={0};
    system("cls");
